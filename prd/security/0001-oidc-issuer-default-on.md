...
- Capture guidance/communication for existing clusters which don't have OIDC issuer on
...
**Pros:**
- Provides a secure default while allowing flexibility for customers to opt-out if they are concerned about the issuer URL being publicly accessible endpoint.
...
- The `oidcIssuerProfile` object will going forward only include the `issuerURL` field to allow customers to discover the OIDC issuer endpoint.
...
|-----|---------|---------|---------|--------|
| 1   | Workload identity adoption on clusters requiring authentication to Entra | Percentage of new clusters using workload identity on AKS versions >= 1.35 | 100% adoption rate | High |
...
| 2   | Require all AKS addons, features, and extensions to use workload identity for authentication on clusters running Kubernetes version 1.35 and above | High |
| 3   | Deprecate use of IMDS-based authentication for AKS addons, features, and extensions on clusters running Kubernetes version 1.35 and above | High |

## Test Requirements

| No. | Requirement | Priority  |
|-----|-----------------------------------------------|----------|
| 1   | Automated tests to verify OIDC issuer is enabled by default on all new AKS clusters | High |
| 2   | Automated tests to ensure all AKS addons, features, and extensions use workload identity for authentication on clusters running Kubernetes version 1.35 and above | High |
| 3   | Tests to confirm IMDS-based authentication is deprecated for AKS addons, features, and extensions on clusters running Kubernetes version 1.35 and above | High |
| 4   | Performance tests to ensure OIDC issuer enablement does not degrade cluster creation time | Medium |
| 5   | Integration and end-to-end tests covering workload identity scenarios across all supported cluster configurations | Medium |

# Dependencies and risks 

| No. | Requirement or Deliverable | Giver Team / Contact |
|-----|---------|---------|
| 1   | OIDC issuer availability on all AKS clusters | AKS Security Team |
| 2   | Updated Azure CLI and PowerShell modules | AKS Security Team |
...
|     | &nbsp;&nbsp;&nbsp;&bull; [AKS Workload Identity overview](https://learn.microsoft.com/azure/aks/workload-identity-overview) |  |
|     | &nbsp;&nbsp;&nbsp;&bull; [Enable OIDC issuer for AKS](https://learn.microsoft.com/azure/aks/oidc-issuer) |  |
|     | &nbsp;&nbsp;&nbsp;&bull; [AKS cluster creation and configuration](https://learn.microsoft.com/azure/aks/cluster-configuration) |  |
|     | &nbsp;&nbsp;&nbsp;&bull; [Azure Key Vault Secrets Provider integration](https://learn.microsoft.com/azure/aks/csi-secrets-store-identity-access) |  |
|     | &nbsp;&nbsp;&nbsp;&bull; [AKS release notes](https://learn.microsoft.com/azure/aks/release-notes) |  |
|     | &nbsp;&nbsp;&nbsp;&bull; [AKS FAQ](https://learn.microsoft.com/azure/aks/faq) |  |
